...
public static class IdentityApiEndpointRouteBuilderExtensions
{
        ...
        where TUser : class, new()
    {
        ...
        {
            accountGroup.MapGet("/info", async Task<Results<Ok<InfoResponseExtended>, ValidationProblem, NotFound>>
           (ClaimsPrincipal claimsPrincipal, [FromServices] IServiceProvider sp) =>
            {
                ...
                return TypedResults.Ok(await CreateInfoResponseExtendedAsync(user, userManager));
            });
        }

        if (!configureOptions.ExcludeInfoPost)
        {
            accountGroup.MapPost("/info", async Task<Results<Ok<InfoResponseExtended>, ValidationProblem, NotFound>>
    ...
    private static async Task<InfoResponseExtended> CreateInfoResponseExtendedAsync<TUser>(TUser user, UserManager<TUser> userManager)
        where TUser : class
    {
        return new()
        {
            Email = await userManager.GetEmailAsync(user) ?? throw new NotSupportedException($"Users must have an {nameof(InfoResponseExtended.Email)}."),
            Username = await userManager.GetUserNameAsync(user) ?? throw new NotSupportedException($"Users must have a {nameof(InfoResponseExtended.Username)}."),
